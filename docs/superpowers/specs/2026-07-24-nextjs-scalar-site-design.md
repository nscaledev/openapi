...
- **A simple list below the banner, not a card grid** (refined after the first pass): one row per published service, a leading icon plus title/version on the left, and a row of icon+label action buttons on the right (Reference, YAML, JSON, Changelog, and Docs when a Mintlify link exists) — built from `@nscaledev/ui`'s `Button` component with a Heroicon child, matching how the library's own consumers pair icons with buttons. Simpler than a card grid and scales better as more services publish.
